...
function ingestCorpus(corpusDir, db) {
  ...
  const insertPassage = db.prepare(
    "INSERT INTO passages (document_id, ordinal, content) VALUES (?, ?, ?)"
  );
  ...
  const upsertOne = db.transaction((file) => {
    ...
    if (!existing) {
      ...
      splitPassages(parsed.content).forEach((content, ordinal) =>
        insertPassage.run(file.filename, ordinal, content)
      );
      counts.added++;
